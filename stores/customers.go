...
func (c *customerStore) filterWhere(filter CustomerListFilter, args []interface{}) (string, []interface{}) {
	...
	if filter.FirstName != "" {
		...
		whereConditions = append(whereConditions, fmt.Sprintf("firstName LIKE $%d", len(args)))
	}
	if filter.LastName != "" {
		args = append(args, filter.LastName+"%")
		whereConditions = append(whereConditions, fmt.Sprintf("lastName LIKE $%d", len(args)))
	}
